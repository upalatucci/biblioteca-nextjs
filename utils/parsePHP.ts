export const parsePHP = (str: string, allowedClasses = {}): string[] => {
  let offset = 0;
  const values = [null];
  const specialNums = { INF: Infinity, "-INF": -Infinity, NAN: NaN };

  const kick = (msg: string, i = offset) => {
    throw new Error(`Error at ${i}: ${msg}\n${str}\n${" ".repeat(i)}^`);
  };
  const read = (expected: string, ret) =>
  ...
  function readArray() {
    ...
    return Object.keys(obj).some((key, i) => key != i.toString())
      ? obj
      : Object.values(obj);
  }
